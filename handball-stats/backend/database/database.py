...
def update_competition_progress(conn, competition_id: int, progress: int, step: str = None, status: str = None):
    ...
    if not conn or not competition_id:
        print(f"[DEBUG] update_competition_progress: conn={conn is not None}, competition_id={competition_id}")
        return
    
    try:
        # Clean emojis from text to avoid Windows encoding errors
        step_clean = step.encode('ascii', 'ignore').decode('ascii') if step else step
        status_clean = status.encode('ascii', 'ignore').decode('ascii') if status else status
        print(f"[DEBUG] Updating competition {competition_id}: progress={progress}%, step='{step_clean}', status='{status_clean}'", flush=True)
        ...
        with conn.cursor() as cur:
            ...
            if step is not None:
                ...
                params.append(step_clean)  # Use cleaned version
                
            if status is not None:
                query_parts.append("scraping_status = %s::\"ScrapingStatus\"")
                params.append(status_clean)  # Use cleaned version
            
            # Add timestamp update
            query_parts.append("updated_at = NOW()")
            query_parts.append("last_scraped_at = NOW()")
            
            query = f"UPDATE competitions SET {', '.join(query_parts)} WHERE id = %s"
            params.append(competition_id)
            
            print(f"[DEBUG] SQL: {query}", flush=True)
            # Skip printing params to avoid emoji encoding errors
            print(f"[DEBUG] Executing update with {len(params)} parameters", flush=True)
            
            cur.execute(query, tuple(params))
            rows_affected = cur.rowcount
            conn.commit()
            
            print(f"[DEBUG] Progress update successful - {rows_affected} row(s) affected")
...
def parse_date_str(date_str: str) -> datetime | None:
    ...
    print(f"    -> [WARN] Impossible de parser la date: {date_str}")
    return None
...
def insert_match_stats(conn, match_data: Dict[str, Any], equipe_cible_id: int, nom_equipe_cible: str, competition_id: int = None):
    ...
    with conn.cursor() as cur:
        try:
            ...
            conn.commit()
            print(f"    -> [OK] Match inséré/mis à jour. ID: {match_id} - Score: {score}")
    ...
    print(f"    -> Insertion des statistiques de {nom_equipe_cible}...")
    ...
    if stat_inserts:
        ...
        with conn.cursor() as cur:
            try:
                ...
                conn.commit()
                total_buts = sum(stat[2] for stat in stat_inserts)  # Buts sont à l'index 2
                print(f"    -> [OK] {len(stat_inserts)} statistiques insérées ({total_buts} buts au total)")
                return True
            except Exception as e:
