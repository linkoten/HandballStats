...
# Configuration de l'encodage pour éviter les problèmes sous Windows
import codecs
...
# Support pour exécution en ligne de commande
def main():
    """Point d'entrée pour l'exécution en ligne de commande"""
    parser = argparse.ArgumentParser(description='Handball Scraper Service')
    parser.add_argument('--competition-ids', type=str, required=True, 
                       help='IDs des compétitions séparés par des virgules')
    
    args = parser.parse_args()
    
    # Parser les IDs
    try:
        competition_ids = [int(id.strip()) for id in args.competition_ids.split(',') if id.strip()]
        print(f"SCRAPING - Démarrage du scraping pour les compétitions: {competition_ids}")
        
        # Console.log des données reçues
        print(f"SCRAPING - Données reçues du serveur Node.js:")
        print(f"   - Nombre de compétitions: {len(competition_ids)}")
        print(f"   - IDs: {competition_ids}")
        print(f"   - Répertoire de travail: {os.getcwd()}")
        print(f"   - Arguments système: {sys.argv}")
        
        # Simuler le scraping pour le moment
        for i, comp_id in enumerate(competition_ids):
            print(f"SCRAPING - Traitement compétition ID {comp_id} ({i+1}/{len(competition_ids)})")
            
            # Simulation du travail de scraping
            print(f"   ETAPE - Connexion à FFHandball...")
            time.sleep(1)
            
            print(f"   ETAPE - Téléchargement des données...")
            time.sleep(2)
            
            print(f"   ETAPE - Sauvegarde en base de données...")
            time.sleep(1)
            
            print(f"   SUCCESS - Compétition {comp_id} terminée !")
        
        print(f"SUCCESS - Scraping terminé avec succès pour {len(competition_ids)} compétition(s)")
        
    except ValueError as e:
        print(f"ERROR - Erreur parsing IDs: {e}")
        sys.exit(1)
...
# FastAPI app pour utilisation en service web  
